...
const createSchema = ({
  ...
  addSchemaTypes([schema.createEnumType({
    ...
    [TYPENAMES.IMAGE]: {
      transformedSrc: {
        ...
        resolve({
          ...
          const transforms = [];
          if (maxWidth && maxHeight) transforms.push(`_${maxWidth}x${maxHeight}`);else if (maxWidth) transforms.push(`_${maxWidth}x`);else if (maxHeight) transforms.push(`_x${maxHeight}`);
